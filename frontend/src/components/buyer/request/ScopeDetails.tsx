...
export function ScopeDetails ({ description, type, quantity, onChange }: ScopeDetailsProps): JSX.Element {
  return (
    <div className="space-y-8">
      ...
      <div>
        <label className="text-xs font-semibold text-[var(--core-color-text-muted)]">Description</label>
        <textarea
          rows={4}
          value={description ?? ''}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label className="text-xs font-semibold text-[var(--core-color-text-muted)]">Category</label>
          <select
            value={type ?? ''}
        ...
        <div>
          <label className="text-xs font-semibold text-[var(--core-color-text-muted)]">Quantity</label>
          <input
            type="number"
            value={quantity ?? ''}
      ...
      {description && (
        <div className="flex items-start gap-3 rounded-sm bg-[var(--core-color-surface-subtle)] px-4 py-3">
          <Sparkles className="mt-0.5 h-4 w-4 text-[var(--core-color-ai-primary)]" aria-hidden="true" />
