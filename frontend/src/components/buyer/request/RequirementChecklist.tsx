...
export function RequirementChecklist ({ category, mustHaves, compliance, onChange }: RequirementChecklistProps): JSX.Element {
  ...
  const complianceOptions = COMPLIANCE_OPTIONS.map((item) => {
    ...
    return (
      ...
      >
        <span className={clsx('h-2 w-2 rounded-full', selected ? 'bg-white' : 'bg-[var(--core-color-border-medium)]')} aria-hidden="true" />
        {item}
      </button>
    )
  })
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <p className="text-xs font-semibold text-[var(--core-color-text-muted)]">Must-have capabilities</p>
      ...
      <div className="space-y-4">
        <p className="text-xs font-semibold text-[var(--core-color-text-muted)]">Compliance & risk</p>
