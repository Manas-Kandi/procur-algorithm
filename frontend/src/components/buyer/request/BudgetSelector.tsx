...
const OPTIONS: Array<{
...
}> = [
  {
    ...
    description:
      'Provide the amount so we can target matching offers immediately.',
  },
  {
    value: 'range',
    label: 'I have a range',
    description:
      'Give us a floor and ceiling. We will balance best value vs. savings.',
  },
  {
    value: 'see-pricing',
...
export function BudgetSelector({
  ...
  currency = 'USD',
  onChange,
}: BudgetSelectorProps): JSX.Element {
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-[var(--core-color-text-muted)]">
          This anchors sourcing strategy and policy routing.
        </p>
      </div>
            ...
            >
              <input
                ...
                onChange={() => {
                  ...
                  const reset =
                    nextMode === 'see-pricing' || nextMode === 'need-approval'
                  onChange({
                    mode: nextMode,
                    budgetMin: reset ? undefined : budgetMin,
                    budgetMax: reset ? undefined : budgetMax,
                  })
                }}
              ...
              <div className="leading-tight">
                <p
                  className={clsx(
                    'text-sm',
                    isSelected
                      ? 'font-semibold text-[var(--core-color-text-primary)]'
      ...
      {(mode === 'see-pricing' || mode === 'need-approval') && (
        <div className="rounded-sm bg-[var(--core-color-surface-subtle)] px-4 py-4 text-sm text-[var(--core-color-text-secondary)]">
          We’ll surface benchmark data for {currency} spend before finalizing
          your budget. Expect a quick policy preview next.
        </div>
      )}
    </div>
  )
