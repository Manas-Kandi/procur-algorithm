...
export function SourcingProgress({
  ...
  responded = 0,
  negotiating = 0,
}: SourcingProgressProps): JSX.Element {
  const steps = [
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        {status === 'completed' ? (
          <CheckCircle2
            className="h-5 w-5 text-[var(--core-color-data-positive)]"
            aria-hidden="true"
          />
        ) : (
          <Loader2
            className="h-5 w-5 animate-spin text-[var(--core-color-brand-primary)]"
            aria-hidden="true"
          />
        )}
        <div>
          <p className="text-sm font-semibold text-[var(--core-color-text-primary)]">
            {status === 'completed'
              ? 'AI sourcing completed'
              : 'AI sourcing in progress'}
          </p>
          <p className="text-xs text-[var(--core-color-text-muted)]">
            We will notify you as negotiations mature.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {steps.map((step) => {
          const percent = Math.min(
            100,
            Math.round((step.value / step.target) * 100)
          )
          return (
            <div key={step.label}>
