...
-- File: 04_create_rls_policies.sql (FIXED VERSION)
...
-- Get the current user's integer ID from JWT email
-- =====================================================
...
BEGIN
    -- Get email from JWT
    BEGIN
        user_email := NULLIF(TRIM(auth.jwt()->>'email'), '');
...
COMMENT ON FUNCTION get_current_user_id() IS 'Get current user integer ID from JWT email';
...
BEGIN
    ...
    SELECT is_superuser INTO is_super
    FROM user_accounts 
    WHERE id = current_user_id
    AND deleted_at IS NULL;
...
BEGIN
    ...
    SELECT role::TEXT, is_superuser INTO user_role, is_super
    FROM user_accounts 
    WHERE id = current_user_id
    AND deleted_at IS NULL;
...
USING (
    organization_id::TEXT = get_user_organization_id()
    OR is_superuser()
);
...
WITH CHECK (
    (organization_id::TEXT = get_user_organization_id() AND has_role('admin'))
    OR is_superuser()
);
...
USING (
    ...
    OR (organization_id::TEXT = get_user_organization_id() AND has_role('admin'))
    OR is_superuser()
)
WITH CHECK (
    id = get_current_user_id()
...
USING (
    EXISTS (
        ...
        AND user_accounts.organization_id::TEXT = get_user_organization_id()
