-- =====================================================
-- CLEAN RLS IMPLEMENTATION - NO TYPE ERRORS
-- =====================================================
...
BEGIN
    RETURN COALESCE((
        SELECT is_superuser FROM user_accounts 
        WHERE id = get_current_user_id()
        AND deleted_at IS NULL
    ), false);
EXCEPTION WHEN OTHERS THEN
...
BEGIN
    RETURN COALESCE((
        SELECT (role = required_role OR is_superuser = true)
        FROM user_accounts 
        WHERE id = get_current_user_id()
        AND deleted_at IS NULL
    ), false);
...
USING (organization_id = get_user_organization_id() AND has_role('admin'))
WITH CHECK (organization_id = get_user_organization_id() AND has_role('admin'));
...
USING (organization_id = get_user_organization_id() OR is_superuser() OR id = get_current_user_id());

CREATE POLICY user_insert ON user_accounts FOR INSERT
WITH CHECK ((organization_id = get_user_organization_id() AND has_role('admin')) OR is_superuser());

CREATE POLICY user_update ON user_accounts FOR UPDATE
USING (id = get_current_user_id() OR (organization_id = get_user_organization_id() AND has_role('admin')) OR is_superuser())
WITH CHECK (id = get_current_user_id() OR (organization_id = get_user_organization_id() AND has_role('admin')) OR is_superuser());

CREATE POLICY user_delete ON user_accounts FOR DELETE
USING ((organization_id = get_user_organization_id() AND has_role('admin')) OR is_superuser());
...
USING (user_id = get_current_user_id() OR has_role('approver') OR has_role('admin') OR is_superuser())
WITH CHECK (user_id = get_current_user_id() OR has_role('approver') OR has_role('admin') OR is_superuser());

CREATE POLICY req_delete ON requests FOR DELETE
USING (user_id = get_current_user_id() OR has_role('admin') OR is_superuser());

-- =====================================================
-- POLICIES: vendor_profiles
-- =====================================================
...
USING (organization_id = get_user_organization_id() OR organization_id IS NULL OR is_superuser());
